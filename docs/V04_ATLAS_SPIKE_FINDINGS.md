...
did not. `ibl-anatomy` now validates and exposes the boundary and provides batched
...
payload; `ibl-datoviz` handles rendering and interaction, while `ibl-anatomy` supplies the
